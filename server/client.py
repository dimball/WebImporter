...
#             self.sock.close()
class Client(hfn.c_HelperFunctions):
    def __init__(self,ip, port, Tasks, type):
        self.connected = False
        self.ip = ip
        self.port = port
        self.Tasks = Tasks
        ...
        try:
            logging.debug("Attempting to register on sync server at %s:%s", self.ip, self.port)
            #self.m_send(self.m_create_data("/syncserver/v1/server/register",self.Tasks.WorkData["local_serverport"]))
            self.connection = create_connection("ws://" + self.ip + ":" + self.port + "/" + type)
            self.connected = True
        except:
